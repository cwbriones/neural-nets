...
std::pair<Dataset, Dataset> MnistLoader::load() {
    ...
    auto training_data = load_dataset(training_path, training_labels);
    std::cout << std::endl;
    std::cout << "Loading test data." << std::endl;
    auto test_data     = load_dataset(test_path, test_labels);
...
VectorList MnistLoader::load_images(const std::string& path) {
        ...
        for (int i = 0; i < number_of_images; ++i) {
            ...
            for (int r = 0; r < rows; ++r) {
                for (int c = 0; c < cols; ++c) {
                    ...
                    image_data(r * rows + c) = tmp;
                }
            }
            images.push_back(std::move(image_data));
        }
    } else {
...
VectorList MnistLoader::load_labels(const std::string& path) {
    std::ifstream file(path);
